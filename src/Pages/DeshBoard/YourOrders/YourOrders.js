...
const YourOrders = () => {
    ...
    useEffect(() => {
        fetch(`https://jutecrafts-server1.onrender.com/order?email=${user.email}`).then(res => res.json()).then(data => setOrders(data))
    }, [orders]);
    const handlePay = (id) => {
        history(`/dashboard/payment/${id}`)
    }
    ...
    return (
        <div>
            ...
            <Button sx={{ my: 5 }} variant="contained" onClick={() => history('/dashboard/payment')}>Check Out</Button>
        </div>
    );
};
